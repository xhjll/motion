...
# # 创建输出视频文件
# output_video = cv2.VideoWriter('output_video1.mp4', cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height), False)

# # 定义变量，用于计算当前帧与前一帧的加权平均  
# prev_frame = None  
# weight = 0.5  
...
#      # 如果存在前一帧，计算当前帧的G通道数据为灰度图像与前一帧的加权平均  
#     if prev_frame is not None:       
#         g_channel = cv2.addWeighted(gray_frame, weight, prev_frame, 1 - weight, 0) 
#         prev_frame = gray_frame.copy() 
#     else:  
#         g_channel = gray_frame.copy()  
  
#     # R通道的数据全是0  
#     r_channel = np.zeros_like(gray_frame)  

#     b_channel = gray_frame.copy() 
